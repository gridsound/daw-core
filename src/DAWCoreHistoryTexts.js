...
class DAWCoreHistoryTexts {
	...
	static #texts = new Map( [
		...
		[ "blocks", {
			...
			moveBlocks: len => [ "arrows", `move ${ GSUplural( len, "block" ) }` ],
			selectBlocks: len => [ "mouse", `select ${ GSUplural( len, "block" ) }` ],
			removeBlocks: len => [ "erase", `remove ${ GSUplural( len, "block" ) }` ],
			cropEndBlocks: len => [ "crop", `crop-end ${ GSUplural( len, "block" ) }` ],
		...
		[ "keys", {
			...
			moveKeys: ( pat, len ) => [ "arrows", `keys: move ${ GSUplural( len, "key" ) } in "${ pat }"` ],
			removeKeys: ( pat, len ) => [ "erase", `keys: remove ${ GSUplural( len, "key" ) } in "${ pat }"` ],
			selectKeys: ( pat, len ) => [ "mouse", `keys: select ${ GSUplural( len, "key" ) } in "${ pat }"` ],
			cropEndKeys: ( pat, len ) => [ "crop", `keys: crop ${ GSUplural( len, "key" ) } in "${ pat }"` ],
			redirectKey: ( pat, b ) => [ "glissando", `${ b ? "add" : "remove" } a glissando in "${ pat }"` ],
			unselectKey: pat => [ "mouse", `keys: unselect ${ GSUplural( 1, "key" ) } in "${ pat }"` ],
			unselectAllKeys: ( pat, len ) => [ "mouse", `unselect ${ GSUplural( len, "key" ) } in "${ pat }"` ],
			dropMidiOnKeys: name => [ "file-music", `keys: drop midi "${ name }"` ],
			changeKeysProps: ( pat, prop, len ) => [ "keys", `keys: change ${ GSUplural( len, "key", "'s" ) } ${ prop } in "${ pat }"` ],
			cloneSelectedKeys: ( pat, len ) => [ "keys", `clone ${ GSUplural( len, "key" ) } in "${ pat }"` ],
		} ],
	] );
}
